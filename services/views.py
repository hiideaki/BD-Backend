...
def buscar_coordenador_grupo(request, pgrupo):
    usuario = Usuario.objects.raw('SELECT SERVICES_USUARIO.* FROM SERVICES_USUARIO, SERVICES_COORDENA WHERE SERVICES_USUARIO.ID=SERVICES_COORDENA.USUARIO_ID AND SERVICES_COORDENA.GRUPO_ID=' + pgrupo)
    serializer = SerializerUsuario(usuario, many=True)
    return Response(serializer.data)
...
def buscar_membros_grupo(request, pgrupo):
    usuario = Usuario.objects.raw('SELECT SERVICES_USUARIO.* FROM SERVICES_USUARIO, SERVICES_PARTICIPA, SERVICES_COORDENA WHERE (SERVICES_USUARIO.ID=SERVICES_PARTICIPA.USUARIO_ID AND SERVICES_PARTICIPA.GRUPO_ID=' + pgrupo + ') OR (SERVICES_USUARIO.ID=SERVICES_COORDENA.USUARIO_ID AND SERVICES_COORDENA.GRUPO_ID=' + pgrupo + ')')
    serializer = SerializerUsuario(usuario, many=True)
    return Response(serializer.data)
